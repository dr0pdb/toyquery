 ...
 public:
  ...
  void ToStr(const T& pVal, std::string& pStr) const {
    if (typeid(T) == typeid(int) || typeid(T) == typeid(long) || typeid(T) == typeid(long long) ||
        typeid(T) == typeid(unsigned) || typeid(T) == typeid(unsigned long) || typeid(T) == typeid(unsigned long long) ||
...
class Document {
 public:
      ...
      if (std::distance(mData.begin(), itRow) > mLabelParams.mColumnNameIdx) {
        if (columnIdx < static_cast<ssize_t>(itRow->size())) {
          ...
          const std::string errStr =
              ...
              " >= " + std::to_string(itRow->size() - (mLabelParams.mRowNameIdx + 1)) + " (number of columns on row index " +
  ...
  void RemoveColumn(const size_t pColumnIdx) {
    const ssize_t columnIdx = pColumnIdx + (mLabelParams.mRowNameIdx + 1);
    for (auto itRow = mData.begin(); itRow != mData.end(); ++itRow) { itRow->erase(itRow->begin() + columnIdx); }
  }
  ...
  ssize_t GetRowIdx(const std::string& pRowName) const {
    if (mLabelParams.mRowNameIdx >= 0) {
      if (mRowNames.find(pRowName) != mRowNames.end()) { return mRowNames.at(pRowName) - (mLabelParams.mColumnNameIdx + 1); }
    }
    return -1;
  }
  ...
  void SetCell(const size_t pColumnIdx, const size_t pRowIdx, const T& pCell) {
    ...
    if ((columnIdx + 1) > GetDataColumnCount()) {
      for (auto itRow = mData.begin(); itRow != mData.end(); ++itRow) { itRow->resize(columnIdx + 1); }
    }
  ...
  void SetColumnName(size_t pColumnIdx, const std::string& pColumnName) {
    ...
    if (rowIdx >= static_cast<int>(mData.size())) { mData.resize(rowIdx + 1); }
    auto& row = mData[rowIdx];
    if (columnIdx >= static_cast<int>(row.size())) { row.resize(columnIdx + 1); }
  ...
  void SetRowName(size_t pRowIdx, const std::string& pRowName) {
    ...
    if (rowIdx >= static_cast<int>(mData.size())) { mData.resize(rowIdx + 1); }
    auto& row = mData[rowIdx];
    if (mLabelParams.mRowNameIdx >= static_cast<int>(row.size())) { row.resize(mLabelParams.mRowNameIdx + 1); }
 ...
 private:
    ...
    if ((bom2b == bomU16le) || (bom2b == bomU16be)) {
      ...
      if (mIsLE) {
        wstream.imbue(std::locale(
            wstream.getloc(),
            new std::
                codecvt_utf16<wchar_t, 0x10ffff, static_cast<std::codecvt_mode>(std::consume_header | std::little_endian)>));
      } else {
  ...
  void ParseCsv(std::istream& pStream, std::streamsize p_FileLength) {
    ...
    if ((mLabelParams.mColumnNameIdx >= 0) && (static_cast<ssize_t>(mData.size()) > mLabelParams.mColumnNameIdx)) {
      int i = 0;
      for (auto& columnName : mData[mLabelParams.mColumnNameIdx]) { mColumnNames[columnName] = i++; }
    }
